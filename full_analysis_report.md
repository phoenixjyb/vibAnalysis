...
> **TOC rule:** All `##` chapters and all `###` subsections are listed. Sub-subsections (`####`) are not listed.
> §6 subsections are named Option A/B/C (not numbered) — this is noted but not changed here.

1. [Platform Specifications](#1-platform-specifications-平台参数)
    - [1.1 Chassis geometry](#11-chassis-geometry-底盘构型)
    - [1.2 Critical kinematics — X-configuration correction](#12-critical-kinematics-x-configuration-correction-x构型运动学修正关键)
    - [1.3 True sample rate](#13-true-sample-rate-真实采样率)
2. [Test Datasets](#2-test-datasets-测试数据集)
    - [2.1 Original baseline](#21-original-baseline-原始基线)
    - [2.2 Multi-surface tests](#22-multi-surface-tests-多地面测试)
3. [Multi-Surface RMS Results](#3-multi-surface-rms-results-多地面rms汇总)
    - [3.1 Z-axis RMS — all surfaces, all speeds](#31-z-axis-rms-g-all-surfaces-all-speeds-z轴rmsg所有地面所有速度)
    - [3.2 Surface severity ratios](#32-surface-severity-ratios-vs-indoor-white-tile-地面严酷度比vs室内白砖)
4. [Frequency Analysis](#4-frequency-analysis-频率分析)
    - [4.1 N=11 roller passage (dominant excitation)](#41-n11-per-plate-roller-passage-dominant-mechanical-excitation-n11每板滚子过频主导机械激励)
    - [4.2 Motor cogging (low-speed dominant)](#42-motor-cogging-dominant-at-low-speeds-电机齿槽低速主导)
    - [4.3 Cement structural resonance (~79 Hz)](#43-cement-structural-resonance-79-hz-水泥路结构共振约79-hz)
    - [4.4 Complete frequency table](#44-complete-frequency-table-all-speeds-完整频率表所有速度)
5. [Low-Speed Resonance Problem](#5-low-speed-resonance-problem-低速共振问题)
    - [5.1 Discovery](#51-discovery-发现)
    - [5.2 Full low-speed transmissibility](#52-full-low-speed-transmissibility-低速段完整传递率)
6. [Mitigation Options for Low-Speed Resonance](#6-mitigation-options-低速共振缓解方案)
    - [Option A — Increase damping only](#option-a-increase-damping-ratio-only-keep-fn-4-hz-方案a仅增大阻尼比保持fn4-hz)
    - [Option B — Lower fn](#option-b-lower-fn-shift-resonance-below-operating-range-方案b降低fn将共振移至工况范围以下)
    - [Option C — Combined (recommended)](#option-c-combined-lower-fn-higher-damping-recommended-方案c组合降低fn提高阻尼推荐)
7. [Suspension Design](#7-suspension-design-悬挂设计)
    - [7.1 1-DOF model](#71-1-dof-model-单自由度模型)
    - [7.2 Mass split](#72-mass-split-质量分配)
    - [7.3 Current design parameters (indoor)](#73-current-design-parameters-indoor-use-当前设计参数室内使用)
    - [7.4 Predicted output — indoor](#74-predicted-suspension-output-indoor-surface-预测悬挂输出室内地面)
    - [7.5 Surface-specific requirements](#75-surface-specific-suspension-requirements-按地面类型的悬挂需求)
    - [7.6 Performance at 1.5 m/s](#76-suspension-performance-at-15-ms-t-0112-15-ms时悬挂性能t0112)
8. [Wheel Comparison: 6-inch N=9 vs 5-inch N=11](#8-wheel-comparison-车轮对比)
    - [8.1 Specifications](#81-specifications-规格对比)
    - [8.2 Transmissibility comparison](#82-transmissibility-comparison-fn-4-hz-ζ-04-传递率对比fn4-hzζ04)
    - [8.3 Head-to-head verdict](#83-head-to-head-verdict-综合评定)
    - [8.4 Verdict](#84-verdict-结论)
    - [8.5 New wheel — updated frequency formulas](#85-new-wheel-updated-frequency-formulas-新轮更新后的频率公式)
9. [Conclusions & Recommendations](#9-conclusions-recommendations-结论与建议)
    - [9.1 Confirmed findings](#91-confirmed-findings-high-confidence-已确认结论高置信度)
    - [9.2 Design recommendations](#92-design-recommendations-设计建议)
    - [9.3 Anti-patterns](#93-anti-patterns-never-do-these-反模式绝对不要做的事)
10. [Adding Mass to Shift Resonance](#10-adding-mass-to-shift-resonance-增加质量以移频)
    - [10.1 The idea](#101-the-idea-想法)
    - [10.2 The fundamental constraint](#102-the-fundamental-constraint-根本限制)
    - [10.3 Computed results](#103-computed-results-计算结果)
    - [10.4 The battery (4 kg)](#104-the-battery-4-kg-specifically-电池4-kg具体效果)
    - [10.5 How to use battery mass correctly](#105-how-to-correctly-use-battery-mass-如何正确利用电池质量)
    - [10.6 Why ballast is inefficient](#106-why-ballast-is-inefficient-为何压载低效)
11. [Pneumatic Tyres vs Omni Wheels](#11-pneumatic-tyres-vs-omni-wheels-充气轮胎与全向轮对比)
    - [11.1 What changes in the vibration model](#111-what-changes-in-the-vibration-model-振动模型的变化)
    - [11.2 Two-DOF system](#112-two-dof-system-二自由度系统)
    - [11.3 Contact patch filtering](#113-contact-patch-filtering-接触面积过滤)
    - [11.4 Suspension design implications](#114-suspension-design-implications-对悬挂设计的影响)
    - [11.5 Head-to-head: vibration vs mobility](#115-head-to-head-vibration-vs-mobility-全面对比振动性能与运动能力)
    - [11.6 Verdict](#116-verdict-结论)
12. [Sandwich Layer — Four-Option Comparative Analysis](#12-sandwich-layer-four-option-comparative-analysis-机体结构夹层减振设计)
    - [12.1 Concept](#121-concept-设计概念)
    - [12.2 Option A — Silicon Bumper Pad](#122-option-a-silicon-bumper-pad-in-series-方案a硅胶减振垫串联叠加)
    - [12.3 Option B — Air Spring Damper](#123-option-b-air-spring-damper-方案b空气弹簧阻尼器)
    - [12.4 Option C — Metal Wire-Mesh Isolator](#124-option-c-metal-wire-mesh-isolator-alj-89b-方案c全金属丝网减振器alj-89b)
    - [12.5 Option D — Wire Rope Isolator](#125-option-d-wire-rope-isolator-834835836-方案d钢丝绳减振器)
    - [12.6 Comparative Summary](#126-comparative-summary-综合对比)
    - [12.7 Recommendation & Model Selection](#127-recommendation-model-selection-推荐方案与型号选择)
    - [12.8 Complete Structural Design](#128-complete-sandwich-isolator-structural-design-完整夹层隔振器结构设计)
...
13. [Additional Isolator Candidates — Extended Evaluation](#13-additional-isolator-candidates-extended-evaluation-新增隔振器候选方案扩展评估)
    - [13.1 DPA Cylindrical Rubber Isolator](#131-dpa-cylindrical-rubber-isolator-圆柱形减振器-dpa型)
    - [13.2 E-type Protector Isolator](#132-e-type-protector-isolator-e型保护式隔振器)
    - [13.3 SHEDA / SHEDC Cylindrical Rubber](#133-sheda-shedc-narrow-mid-cylindrical-rubber-腰型及圆柱型减振螺栓)
    - [13.4 ST-type Air Spring Mount](#134-st-type-air-spring-mount-st型空气弹簧减振器)
    - [13.5 ZT-type Rubber Spring Mount](#135-zt-type-rubber-spring-mount-zt型橡胶弹簧减振器)
    - [13.6 AVC Wire Rope Isolator](#136-avc-aisi-316-stainless-steel-wire-rope-isolator-avc不锈钢钢丝绳减振器)
    - [13.7 Comparative Transmissibility — All Options](#137-comparative-transmissibility-all-options-全选项传递率对比)
    - [13.8 Overall Ranking — All 10 Options](#138-overall-ranking-across-all-10-options-evaluated-全部10款方案综合排名)
14. [Dual-Accelerometer Results — Chassis vs End-Effector](#14-dual-accelerometer-results-chassis-vs-end-effector-双加速度计对比底盘与末端执行器)
    - [14.1 Key finding: arm as mechanical low-pass filter](#141-key-finding-the-arm-is-a-mechanical-low-pass-filter-机械臂是机械低通滤波器)
    - [14.2 Z-axis RMS: chassis vs end-effector](#142-z-axis-rms-chassis-vs-end-effector-z轴rms底盘与末端执行器)
    - [14.3 Total 3-axis RMS](#143-total-3-axis-rms-the-arm-has-significant-horizontal-motion-三轴总rms机械臂水平振动不可忽视)
    - [14.4 Interpretation of results](#144-interpretation-of-results-结果解读)
    - [14.5 Design implications](#145-design-implications-设计影响)
    - [14.6 EE absolute vibration (current state, no suspension)](#146-end-effector-absolute-vibration-without-suspensionsandwich-current-state-末端执行器绝对振动当前无悬挂夹层状态)
    - [14.7 Per-axis (X/Y/Z) RMS](#147-per-axis-xyz-rms-chassis-vs-end-effector-各轴xyzrms底盘与末端执行器)
...
15. [RC Coil-Over Shock — Candidate Evaluation & Action Items](#15-rc-coil-over-shock-candidate-evaluation-action-items-rc同轴减震器候选评估与待办事项)
    - [15.1 Candidate specifications](#151-candidate-specifications-候选方案规格)
    - [15.2 Why Candidate C cannot be used with sandwich](#152-why-candidate-c-cannot-be-used-with-sandwich-hardware-为何c不能与夹层硬件配合)
    - [15.3 Updated status and action items](#153-updated-status-and-action-items-更新状态与待办事项)
16. [Suspension Mechanical Design — Geometry Recommendation](#16-suspension-mechanical-design-geometry-recommendation-悬挂机械结构设计推荐)
    - [16.1 Options considered](#161-options-considered-候选方案分析)
    - [16.2 Recommended geometry: Trailing arm](#162-recommended-geometry-trailing-arm-推荐方案拖臂式)
    - [16.3 Summary verdict](#163-summary-verdict-总结)
...
#### Reference hardware sample assessment / 参考实物样品评估

The sample shown in `ref-info/suspension-sample.jpg` is a triangulated black-anodised aluminium bracket with an RC coil-over shock mounted inside the frame.

Key observations:

| Feature / 特征 | Observed / 观察结果 | Compatibility / 兼容性 |
|---|---|---|
| Linkage type / 连杆类型 | Single trailing arm with triangulated bracket — pivot at base plate | ✓ Matches recommended geometry |
| Coil-over position / 减震器位置 | Mounted between arm and top of bracket, slight inboard angle | ✓ Axial loading preserved; slight angle (< 15°) reduces effective k by cos²θ ≈ 0.97 — negligible |
| Bracket material / 支架材料 | Anodised aluminium plate, bent/welded triangular frame | ✓ Matches fabrication recommendation (3–4 mm 6061-T6 Al) |
| Torsional stiffness / 抗扭刚度 | Triangular bracing on bracket | ✓ Good — prevents arm twist under lateral wheel loads |
| Pivot location / 转轴位置 | At base-plate attachment to chassis | ✓ Standard trailing arm pivot — allows clean vertical arc |
| Wheel hub attachment / 轮毂连接 | Far end of arm | ✓ Direct wheel centre mount; arm length appears ~120–140 mm |

**Assessment: compatible and recommended.** The sample geometry is a trailing arm with integrated coil-over — exactly the configuration recommended in §16.2. The triangular bracket bracing is a good practice for reducing torsional compliance. The slight coil-over angle is acceptable (< 3% k loss). This sample or a close derivative is suitable for fabrication.

**评估：兼容，推荐。** 该样品为带同轴减震器的拖臂结构，与§16.2推荐方案完全一致。三角形支架加强抗扭刚度是良好做法。减震器轻微倾斜可接受（刚度损失<3%）。该样品或近似结构适合用于加工制造。
